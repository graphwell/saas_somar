...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 h-screen w-[240px] z-50 flex flex-col
      ...
      >
        ...
        <button
          className="md:hidden absolute top-4 right-4 p-1.5 rounded-lg text-[#9CA3AF] hover:text-white hover:bg-white/10 transition-colors"
          onClick={onClose}
          ...
          <div className="space-y-1">
            {secondaryItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-3 px-3 py-2 rounded-[8px] text-sm font-medium text-[#9CA3AF] hover:bg-white/5 hover:text-white transition-colors"
              >
                <item.icon size={18} />
                {item.label}
              </Link>
